...
//	@Atomic	- only called inside a atomic method (we should avoid nested transactions)
...
//				System.out.println("set_best: " + best + " <-> " + MinTourLen);
	...
	// Implemented by Vasco Pessanha
	@Atomic
	static private void set_best2(int best, int[] path) {
		if (best < MinTourLen) {
//			if (debug) {
//				System.out.print("(" + Thread.currentThread().getName() + ") ");
//				System.out.println("set_best MinTourLen: " + best + " (old: "
//						+ MinTourLen + ")");
//			}
			MinTourLen = best;
...
//			System.out.println("calc_bound");
		...
		/*
		 * wt1: the value of the edge with the lowest weight from the node we're
		 * testing to another unconnected node. wt2: the value of the edge with
		 * the second lowest weight
		 */
		 ...
		 * that using an edge would not make it impossible for a vertex to have
		 * degree two. (2) Check to make sure that the edge doesn't give some
		 * vertex degree 3.
		 */
...
//			System.out.println("split_tour");
		...
		if (curr.last != (Tsp.TspSize - 1)) {
			...
			for (i = 0; i < Tsp.TspSize; i++) {
				/*
				 * Check: 1. Not already in tour 2. Edge from last entry to node
				 * in graph and 3. Weight of new partial tour is less than cur
				 * min.
				 */
				...
				if (t1 && t2 && t3) {
					...
					/*
					 * If it's shorter than the current best tour, or equal to
					 * the current best tour and we're reporting all min length
					 * tours, put it on the priority q.
					 */
					new_ = Tours[n_ind];
...
//							+ " > " + MinTourLen);
	 ...
	 * find_solvable_tour():
	 * 
	 * Used by both the normal TSP program (called by get_tour()) and the RPC
	 * server (called by RPCserver()) to return the next solvable (sufficiently
	 * short) tour.
	 */
